...
```
 ...
 22. How to redirect **STDOUT** and **STDERR* in bash? \
     - `> /dev/null 2>&1`
 24. What is the difference between **UNIX** and **Linux**.
 25. What is the difference between **Telnet** and **SSH**?
 26. Explain the three load averages and what do they indicate. \\ What command can be used to view the load averages?
 27. Can you name a lower-case letter that is not a valid option for GNU ''ls''?
```
# ls -e
ls: invalid option -- 'e'
...
```
 ...
 14. What is an inode and what fields are stored in an inode?
 15. How to force/trigger a file system check on next reboot?
 16. What is **SNMP** and what is it used for?\
 ...
 17. What is a runlevel and how to get the current runlevel?\
 ...
 19. What is the difference between local and remote port forwarding?\
    - [https://en.wikipedia.org/wiki/Port_forwarding](https///en.wikipedia.org/wiki/Port_forwarding)
 20. What are the steps to add a user to a system without using `useradd` / `adduser`?
 21. What is MAJOR and MINOR numbers of special files?
 22. Describe the `mknod` command and when you'd use it.
 23. Describe a scenario when you get a "filesystem is full" error, but `df` shows there is free space.
 24. Describe a scenario when deleting a file, but `df` not showing the space being freed.
 25. Describe how `ps` works.
 26. What happens to a child process that dies and has no parent process to wait for it and what’s bad about this?
 27. Explain briefly each one of the process states.
 28. How to know which process listens on a specific port?
```
port_number="60734"
...
```
 ...
 29. You run a bash script and you want to see its output on your terminal and save it to a file at the same time.\
     How could you do it?
    - `program 2>&1 | tee logfile`{bash}
 31. Explain what `echo "1" > /proc/sys/net/ipv4/ip_forward` does.
 32. Describe briefly the steps you need to take in order to create and install a valid certificate for the site `https://foo.example.com`
 33. Can you have several HTTPS virtual hosts sharing the same IP?
 34. What is a wildcard certificate?
 35. Which Linux file types do you know?
 36. What is the difference between a process and a thread?\
     And parent and child processes after a fork system call?
 38. What is the difference between ''exec'' and ''fork''?\
 ...
 39. What is `nohup` used for?\
    - [https://en.wikipedia.org/wiki/Nohup](https///en.wikipedia.org/wiki/Nohup)
 40. What is the difference between these two commands?
```
myvar=hello
export myvar=hello
```
 ...
 45. Do you know of any alternative shells?\
     If so, have you used any?
 47. What is a tarpipe (or, how would you go about copying everything, including hardlinks and special files, from one server to another)?
 48. How can you tell if the ''httpd'' package was already installed?
 49. How can you list the contents of a package?
 50. How can you determine which package is better: ''openssh-server-5.3p1-118.1.el6_8.x86_64'' or ''openssh-server-6.6p1-1.el6.x86_64'' ?
...
```
     - File/directory permissions issue - the primary suspect Look at the:\
...
```
 ...
 18. You ran a binary and nothing happened. How would you debug this?\
    - `man strace`
 19. What are **cgroups**?\
     Can you specify a scenario where you could use them?
 21. How can you remove/delete a file with file-name consisting of only non-printable/non-type-able characters?
 22. How can you increase or decrease the priority of a process in Linux?
    - `man nice`
    - `man ionice`
 23.  What are run-levels in Linux?
    - [https://en.wikipedia.org/wiki/Runlevel](https///en.wikipedia.org/wiki/Runlevel)

### Expert Linux Questions

 1. A running process gets ''EAGAIN: Resource temporarily unavailable'' on reading a socket.\
    How can you close this bad socket/file descriptor without killing the process?

### Networking Questions

 1. What is `localhost` and why would `ping localhost` fail?
 2. What is the similarity between `ping` & `traceroute` ?\
    How is `traceroute` able to find the hops.
 4. What is the command used to show all open ports and/or socket connections on a machine?
 5. Is **300.168.0.123* a valid IPv4 address?\
    - No :-D
 6. Which IP ranges/subnets are "private" or "non-routable" ([RFC 1918)](https///tools.ietf.org/html/rfc1918)?
 7. What is a VLAN?\
 ...
 11. What is command used to show the routing table on a Linux box?
 12. A TCP connection on a network can be uniquely defined by 4 things. What are those things?\
    - `src_address:src_port `<-->` dst_address:dst_port`
 13. When a client running a web browser connects to a web server, what is the source port and what is the destination port of the connection?
    - `client_address:src_port(usually > 1023) `<-->` webserver_address:80`
    - `client_address:src_port(usually > 1023) `<-->` webserver_address:443`
 14. How do you add an IPv6 address to a specific interface?
 15. You have added an IPv4 and IPv6 address to interface `eth0`.\
     A ping to the v4 address is working but a ping to the v6 address gives yout the response sendmsg: operation not permitted.\
     What could be wrong?
 17. What is SNAT and when should it be used?\
 ...
 18. Explain how could you ssh login into a Linux system that DROPs all new incoming packets using a SSH tunnel.
 19. How do you stop a DDoS attack?
 20. How can you see content of an ip packet?
 21. What is IPoAC (RFC 1149)?\
